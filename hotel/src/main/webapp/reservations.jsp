...
<%
	...
	int postid = 0;  // 기본값 설정 (예: 0)
	
	if (postidstr != null && !postidstr.trim().equals("")) {
	    try {
	        postid = Integer.parseInt(postidstr);
	    } catch(NumberFormatException e) {
	        // postid가 숫자가 아니면 예외 처리
	        postid = 0; // 또는 에러 메시지 출력/페이지 리다이렉트 등
	    }
	} else {
	    // postid 파라미터가 없으면 에러 처리/리다이렉트/기본값 사용
	    response.sendRedirect("hotels.jsp"); // 또는 에러 페이지로 이동
	    return;
	}
