...
<%
	...
	try {
	    ...
	    pstmt = conn.prepareStatement(sql);
	    pstmt.setInt(1, post_id);        // 1번째 파라미터: parent_id
	    pstmt.setString(2, "ROOM_TYPE"); // 2번째 파라미터: post_type
	    rs = pstmt.executeQuery();
	...
	try{
		conn = DBCPUtil.getConnection();
		String sql = "SELECT title,content FROM posts WHERE  parent_id = ? AND post_type = ?";
		pstmt = conn.prepareStatement(sql);
		pstmt.setInt(1, post_id);
        ...
        <div class="form-container">
            <form action="upload" method="POST" enctype="multipart/form-data">                   
                ...
                <div class="form-section">
                    ...
                    <%if(roomType != null && !roomType.isEmpty()){%>
                        <%for(PostVO d : roomType){ %>
                    <div class="dynamic-section">
                        <div id="roomTypes">
                            <div class="dynamic-item">
                                <div class="form-group">
                                    <label>객실 타입명 *</label>
                                    <input type="text" name="room_titles" value="<%=d.getTitle() %>" required>
                                </div>
                                <div class="form-group">
                                    <label>1박 요금 (원) *</label>
                                    <input type="number" name="room_prices" value="<%=d.getPrice() %>" required>
                                </div>
                                <div class="form-group">
                                    <label>객실 개수 *</label>
                                    <input type="number" name="room_counts" value="<%=d.getRoom_count()%>" required>
                                </div>
                                <div class="form-group">
                                    <label>객실 설명</label>
                                    <input type="text" name="room_contents" value="<%=d.getContent()%>" required>
                                </div>
	        ...
	        <div class="image-preview-container">
	            <div id="imagePreviewGrid" class="image-preview-grid">
	                <%
	                    // 기존 등록 이미지 미리보기 + 삭제 버튼
	                    if (hotelPost != null && hotelPost.getFileName() != null) {
	                        String[] images = hotelPost.getFileName();
	                        for (int i = 0; i < images.length; i++) {
	                %>
	                <div class="image-preview-item" data-img="<%=images[i].trim()%>">
	                    <img src="img/<%=images[i].trim()%>" alt="호텔 이미지">
	                    <button 
	                        type="button"
	                        class="remove-btn"
	                        onclick="removeExistingImage('<%=images[i].trim()%>', this)"
	                        title="이미지 삭제"
	                    >
...
<script>
    // 기존 등록 이미지 미리보기 삭제
    const removeExistingImage = (imageName, btn) => {
        // 1. 미리보기에서 제거
        const previewItem = btn.closest('.image-preview-item');
        if (previewItem) previewItem.remove();

        // 2. 폼에 hidden input 추가 (서버에 삭제 요청)
    ...
    // 새로 선택한 이미지 미리보기
    const previewMultipleImages = (input) => {
        const files = input.files;
        const previewGrid = document.getElementById('imagePreviewGrid');

        // 기존 신규 이미지 미리보기 제거 (기존 DB이미지는 유지)
        // 새 이미지 미리보기만 추가
        Array.from(previewGrid.querySelectorAll('.image-preview-item[data-img]')).forEach(item => item.remove());
            ...
            Array.from(files).forEach((file, i) => {
                ...
                reader.onload = (e) => {
                    ...
                    previewItem.innerHTML = `
                        ...
                        <button type="button" class="remove-btn" onclick="removeImageFromGrid(this, ${i})" title="이미지 삭제">
                            <i class="fas fa-times"></i>
                        </button>
                    `;
    ...
    // 새로 선택한 이미지에서 개별 이미지 제거
    const removeImageFromGrid = (button, index) => {
